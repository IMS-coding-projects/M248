...
function Link({ text, url = null }: { text: string; url?: string | null }) {
    return (
        (url) ? <a className={"underline hover:cursor-pointer"} href={url} target={"_blank"} rel="noopener noreferrer">{text}</a> 
            : 
            <span className={"underline hover:cursor-pointer"} onClick={() => toast("Uh oh!", {description: "This feature is not implemented yet :("})}>{text}</span>
    )
}
